...
function tosatboxes(pre)
  ...
  map(x->convert(HyperBox,collect(values(x.intervals))),sat)
end

function tomixedboxes(pre)
  mixed =  Sigma.mixedsat_tree_data(pre)
  map(x->convert(HyperBox,collect(values(x.intervals))),mixed)
end
...
#       mixed =  Sigma.mixedsat_tree_data(pre)
#       sat =  Sigma.sat_tree_data(pre)
#       mixed_boxes = map(x->convert(HyperBox,collect(values(x.intervals))),mixed)
#       sat_boxes = map(x->convert(HyperBox,collect(values(x.intervals))),sat)
