...
function isapprox{T}(XS::PureRandArray{T}, ys::Array{T},epsilon = DEFAULT_PREC)
  sum(abs(XS - ys)) <= epsilon
end

function isapprox{T}(ys::Array{T}, XS::PureRandArray{T}, epsilon = DEFAULT_PREC)
  sum(abs(ys - XS)) <= epsilon
end

function isapprox{T}(XS::PureRandArray{T}, YS::PureRandArray{T}; epsilon = DEFAULT_PREC)
  sum(abs(XS - YS)) <= epsilon
end

# function isapprox{T}(XS::PureRandVector{T}, ys::Array{T}; epsilon = 0.0001)
#   sum(abs(XS - ys)) <= epsilon
# end
