...
function go()
  ...
  μ = ifelse(uniform(0,1) > 0.5, μ1, μ2)

  params = RandArray(Any[μ, μ1, μ2 ,s])

  npoints = 40
  # Generate random data for testing
  # data = [rand(flip()) ? rand(logistic(0.89, 0.05)) for i = 1:npoints]
  data = [rand(flip()) ? rand(logistic(0.89, 0.05)) : rand(logistic(0.15, 0.05)) for i = 1:npoints]
  ...
  nsamples = 100
  samples = rand((params, sample_rvs), dist(data, sample_rvs; epsilon = 0.001), nsamples; precision=0.001, parallel = true, RandVarType=Sigma.SymbolicRandVar)
end

resultsgo2, statsgo2 = capture(go, [:distance, :sat_check, :post_loop])
